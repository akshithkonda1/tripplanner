...
| **Cloud** | AWS Lambda · API Gateway (REST + WebSocket) · DynamoDB · ElastiCache |
| **AI** | Amazon Bedrock (Claude) — Sam plans and chats in whichever mode you're in |
| **Auth** | Sign in with Apple / Google · JWT |
| **Push** | APNs + FCM via AWS SNS |

The React Native client under `trippy-app/mobile` is the prototype. Native Swift and Kotlin apps in `ios/` and `android/` are the product going forward. Both talk to the same AWS APIs.
...
### 1. Onboarding / Auth
Sign in with Apple (iOS) or Google (Android), email fallback, travel preferences.
...
### 6. Sam (AI)
Talk naturally. Road: “add a national park near Denver.” Flight: “10 days in Japan under $1800 including flights.” Sam is mode-aware. Accept or reject suggestions. Streams over WebSocket.
...
### 9. Booking
Budget-first lodging (hostels, campsites, cheap hotels). Flight Mode also attaches fares to legs.

### 10. Fuel (Road / Hybrid drive legs)
Route fuel cost, cheapest stations, vehicle MPG. Hidden when there are no drive legs.

### 11. Weather
Forecast per stop or city. Drive-day vs outbound-flight alerts.
...
Requires iOS 17+ / Xcode 16. Set `HTTP_API_URL` and `WS_API_URL` in the scheme (or leave the placeholders — the app runs on sample trips).
...
1. **Scaffolding** — SwiftUI + Compose navigation, shared models, sample trips
2. **Auth** — Sign in with Apple / Google, tokens in Keychain / EncryptedSharedPreferences
3. **Trip CRUD** — create / list / edit against the REST API, including travel mode
4. **Itinerary & map** — day timeline + drive polylines and flight arcs
5. **Sam** — WebSocket, streaming, mode-aware Bedrock prompts
6. **Group chat** — realtime, typing, participants
7. **Budget** — expenses, splits, multi-currency
8. **Fuel & weather** — overlay on the road, forecasts per city
9. **Flight Mode** — multi-city legs, fare search, check-in reminders
10. **Booking** — lodging everywhere, flight deep-links
11. **Offline** — SwiftData / Room cache, sync queue
12. **Polish** — haptics, a11y, dark mode, store listings
